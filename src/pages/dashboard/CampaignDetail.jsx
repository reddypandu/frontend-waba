...
const CampaignDetail = () => {
  ...
  const { data: template } = useQuery({
    ...
    queryFn: async () => {
      const { templates } = await apiPost("/api/whatsapp", {
  ...
  reasonMap.forEach((count, reason) =>
    failReasonsGrouped.push({ reason, count }),
  );
  ...
  const getFilteredLogs = (filter) => {
    ...
    if (filter === "delivered")
      return messageLogs.filter((l) =>
        ["delivered", "read", "replied"].includes(l.status),
      );
    if (filter === "read")
  ...
  const stats = [
    ...
    },
    {
      key: "read",
      label: "Read",
      value: statsData?.read || 0,
      color: "text-primary",
    },
    {
      key: "replied",
      label: "Replied",
      value: statsData?.replied || 0,
      color: "text-foreground",
    },
    {
      key: "failed",
    ...
    <div className="space-y-6">
      ...
      {countdown !== null && countdown > 0 && (
        <div className="rounded-xl border-2 border-primary bg-primary/5 p-6 text-center space-y-3 animate-in slide-in-from-top-4 duration-300">
          ...
          <p className="text-5xl font-bold text-primary tabular-nums tracking-tighter">
            {countdown}
          </p>
          <p className="text-sm text-muted-foreground">
            seconds · messages will be sent to {campaign.total_contacts}{" "}
            contacts
          </p>
          <Button variant="destructive" size="sm" onClick={cancelGoLive}>
            Cancel Launch
          </Button>
        </div>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <Separator orientation="vertical" className="h-6" />
          <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${statusStyle.bg} ${statusStyle.text}`}
          >
      ...
      <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            {failedLogs.length > 0 &&
              (campaign.status === "completed" ||
                ...
                >
                  <RotateCcw className="w-3.5 h-3.5" /> Retarget Failed (
                  {failedLogs.length})
                </Button>
              )}
        ...
        {statsData?.sent > 0 &&
          ...
          statsData?.account_status && (
            <div className="rounded-lg border-l-4 border-orange-500 bg-orange-50/50 p-4 space-y-2">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-orange-800 mb-2">
                    {statsData.account_status.sent} messages were sent but{" "}
                    {statsData.account_status.delivered === 0
                      ? "none were delivered"
                      : "very few were delivered"}
                    .
                  </p>
                  ...
                  <ul className="text-xs text-orange-700 space-y-1 ml-4">
                    <li>• Missing payment method in Meta Business Manager</li>
                    <li>
                      • WhatsApp number not verified or messaging not enabled
                    </li>
                    <li>
                      • Account restrictions due to quality rating or policy
                      violations
                    </li>
                    <li>
                      • Check WhatsApp Setup page for account status details
                    </li>
                  </ul>
                </div>
              </div>
            ...
            {stats.map((s) => (
              ...
              >
                <CardContent className="p-4">
                  ...
                  <p
                    className={`text-2xl font-bold font-mono tracking-tight ${s.color}`}
                  >
                    {s.value}
                  </p>
                  {s.key === "failed" && failReasonsGrouped.length > 0 && (
                    <p
                      className="text-[10px] text-destructive mt-1 truncate"
                      title={failReasonsGrouped[0].reason}
                    >
                      {failReasonsGrouped[0].reason}
                    </p>
                  )}
                  {isProcessing &&
                    s.key !== "attempted" &&
      ...
      <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-6 text-sm">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="font-semibold text-primary underline underline-offset-4 decoration-primary/30">
          ...
          {template?.components ? (
            <div className="rounded-xl border border-border bg-emerald-50/20 shadow-inner p-4 relative overflow-hidden">
              ...
              <p className="text-xs font-bold text-emerald-800/60 uppercase mb-2">
                Content Preview
              </p>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="secondary" className="font-mono text-[10px]">
              {campaign.started_at
                ? format(new Date(campaign.started_at), "dd/MM/yyyy HH:mm:ss")
                : "Queued"}
            </Badge>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Badge variant="secondary" className="font-mono text-[10px]">
              {campaign.completed_at
                ? format(new Date(campaign.completed_at), "dd/MM/yyyy HH:mm:ss")
                : "---"}
            </Badge>
          </div>
        </div>
      ...
      <Dialog
        ...
        onOpenChange={(o) => !o && setReportType(null)}
      >
          ...
          {reportType === "failed" && failReasonsGrouped.length > 0 && (
            <div className="space-y-3 mb-4">
              {failReasonsGrouped.map((item, i) => (
                <div
                  key={i}
                  className="bg-destructive/5 border border-destructive/20 rounded-lg p-3 text-xs"
                >
            ...
            <Table>
              <TableHeader className="bg-muted/50">
                <TableRow>
                  ...
                  <TableHead className="text-xs">Reason / Meta ID</TableHead>
              ...
              <TableBody>
                {getFilteredLogs(reportType).map((log, i) => (
                  <TableRow key={log._id}>
                    ...
                    <TableCell className="text-right text-[10px] text-muted-foreground">
                      {format(
                        new Date(log.updatedAt || log.createdAt),
                        "HH:mm:ss",
                      )}
                    </TableCell>
                  </TableRow>
                ...
                {getFilteredLogs(reportType).length === 0 && (
                  <TableRow>
                    ...
                    >
                      No contacts matching this list.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      <AlertDialog
        open={showRetargetConfirm}
        onOpenChange={setShowRetargetConfirm}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will resend the campaign to{" "}
              <strong>{failedLogs.length}</strong> contact(s) that didn't
              receive the message. Estimated cost: ₹{" "}
              {(failedLogs.length * 0.5).toFixed(2)}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
