...
export default function BusinessWorkflows() {
  ...
  const { data: workflows = [], isLoading: workflowsLoading } = useQuery({
    queryKey: ["business-workflows", user?.id],
    queryFn: async () => {
      const data = await apiGet("/api/automation/workflows?type=business");
  ...
  const { data: analyticsData, isLoading: analyticsLoading } = useQuery({
    queryKey: ["business-workflow-analytics", user?.id],
    queryFn: async () => apiGet("/api/automation/workflows/analytics?type=business"),
    enabled: !!user && !profileLoading && canUseWorkflows,
    refetchInterval: 10000,
  });

  const createMutation = useMutation({
    mutationFn: (data) => apiPost("/api/automation/workflows", { ...data, workflow_type: "business" }),
    onSuccess: () => {
      toast({ title: "Business Workflow created!" });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...data }) => apiPut(`/api/automation/workflows/${id}`, { ...data, workflow_type: "business" }),
    onSuccess: () => {
      toast({ title: "Business Workflow updated!" });
      queryClient.invalidateQueries({ queryKey: ["business-workflows", user?.id] });
      queryClient.invalidateQueries({ queryKey: ["business-workflow-analytics", user?.id] });
      closeBuilder();
    },
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, is_active }) => apiPut(`/api/automation/workflows/${id}`, { is_active, workflow_type: "business" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["business-workflows", user?.id] });
      queryClient.invalidateQueries({ queryKey: ["business-workflow-analytics", user?.id] });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => apiDelete(`/api/automation/workflows/${id}?type=business`),
    onSuccess: () => {
      toast({ title: "Business Workflow deleted" });
      queryClient.invalidateQueries({ queryKey: ["business-workflows", user?.id] });
      queryClient.invalidateQueries({ queryKey: ["business-workflow-analytics", user?.id] });
    },
      ...
      <div className="space-y-6">
        <Card className="shadow-sm">
          <CardContent className="p-8 text-center">
            ...
            <h1 className="text-3xl font-bold tracking-tight">Business Workflows</h1>
            <p className="text-muted-foreground mt-1 text-sm">
              Automate bookings, payments, and data collection.
            </p>
            <p className="text-muted-foreground max-w-md mx-auto mt-4 mb-6">
  ...
  return (
    <div className="space-y-6">
      <VisualFlowBuilder
        ...
        isBusinessWorkflow={true}
      />

      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <GitBranch className="h-6 w-6 text-primary" /> Business Workflows
          </h1>
        ...
        <Button onClick={() => openBuilder(null)} className="rounded-xl">
          <Plus className="mr-2 h-4 w-4" /> New Business Workflow
        </Button>
      </div>
