...
const navGroups = [
  ...
  {
    label: "Audience",
    items: [{ title: "Contacts", path: "/dashboard/contacts", icon: Users }],
  },
  {
    label: "Account",
    items: [
...
const DashboardSidebar = () => {
  ...
  return (
    <aside
      className="hidden lg:flex flex-col w-64 h-screen sticky top-0 shrink-0 overflow-y-auto border-r border-sidebar-border"
      ...
      <nav className="flex-1 py-4 px-3 space-y-6 overflow-y-auto">
        {navGroups.map((group) => {
          const items = group.items.filter(
            (item) => item.path !== "/dashboard/admin" || isAdmin,
          );
